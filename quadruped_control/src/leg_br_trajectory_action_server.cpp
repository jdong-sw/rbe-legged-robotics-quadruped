...
public:
    SetTrajectoryAction(std::string name):
        ...
        client("leg_br_pose_action", true),
        actionName(name)
    {
        this->node = node;
...
private:
	...
	double bodyVelocity;
    // -0.1016 -0.1524 0
    // double xOffset = -0.1016;
    // double yOffset = -0.1524;
    // double zOffset = -0.285;
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "leg_br_trajectory_action");
    ROS_INFO("Initialized ros...");

    SetTrajectoryAction actionServer("leg_br_trajectory_action");
