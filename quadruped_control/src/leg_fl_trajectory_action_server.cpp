...
public:
    SetTrajectoryAction(std::string name):
        ...
        client("leg_fl_pose_action", true),
        actionName(name)
    {
        this->node = node;
...
private:
    ...
    geometry_msgs::Point SineTrajectory(double phase)
    {
		...
		ROS_INFO("fl stride length: %f", strideLength);
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "leg_fl_trajectory_action");
    ROS_INFO("Initialized ros...");

    SetTrajectoryAction actionServer("leg_fl_trajectory_action");
