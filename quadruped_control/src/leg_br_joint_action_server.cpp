...
public:
        ...
        actionName(name)
    {
        ...
        this->hipJointPublisher = node.advertise<std_msgs::Float64>("/quadruped/leg_br/hip_joint_position_controller/command", 1);
        this->kneeJointPublisher = node.advertise<std_msgs::Float64>("/quadruped/leg_br/knee_joint_position_controller/command", 1);
        this->ankleJointPublisher = node.advertise<std_msgs::Float64>("/quadruped/leg_br/ankle_joint_position_controller/command", 1);

        ROS_INFO("Subscribing to FKPoseSolver service...");
        this->fkClient = node.serviceClient<quadruped_control::SolveFKPose>("/quadruped/leg_br/fk/pose");
    ...
    void jointStatesCB(const sensor_msgs::JointStateConstPtr& msg)
    {
        ...
        for (int i = 0; i < temp.name.size(); ++i)
        {
            ...
            if (name_i.find("br") != std::string::npos)
            {
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "leg_br_joint_action");

    SetJointAction actionServer("leg_br_joint_action");
    ros::spin();
    return 0;
}
