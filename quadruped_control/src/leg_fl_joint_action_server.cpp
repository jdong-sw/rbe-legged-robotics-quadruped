...
public:
        ...
        actionName(name)
    {
        ...
        this->hipJointPublisher = node.advertise<std_msgs::Float64>("/quadruped/leg_fl/hip_joint_position_controller/command", 1);
        this->kneeJointPublisher = node.advertise<std_msgs::Float64>("/quadruped/leg_fl/knee_joint_position_controller/command", 1);
        this->ankleJointPublisher = node.advertise<std_msgs::Float64>("/quadruped/leg_fl/ankle_joint_position_controller/command", 1);

        ROS_INFO("Subscribing to FKPoseSolver service...");
        this->fkClient = node.serviceClient<quadruped_control::SolveFKPose>("/quadruped/leg_fl/fk/pose");
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "leg_fl_joint_action");

    SetJointAction actionServer("leg_fl_joint_action");
    ros::spin();
    return 0;
}
